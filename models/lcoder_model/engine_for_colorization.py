...
colors = ['white', 'black', 'blue', 'red', 'green', 'yellow', 'grey', 'gray', 'silver', 'gold', 'golden', 'pink',
          'purple', 'brown','cyan','orange', 'tan', 'khaki', 'colors', 'colorful', 'color'] # salmon
out_colors = ['blue', 'red', 'green', 'yellow','pink', 'purple', 'brown','orange'] # salmon

# def train_class_batch_norearange(model, samples, target, cap, criterion, occm_gt , sobel_op=None, occm_loss_w = 0.0001, edge_loss_w = 0.01):
def train_class_batch_norearange(model, samples, target, cap, criterion, occm_gt , sobel_op=None, occm_loss_w = 0., edge_loss_w = 0.):        
    ...
    if edge_loss_w != 0:
        # L1_loss
        fn_edge_loss = torch.nn.L1Loss()
        # 检测边缘
        outputs_edge = torch.cat((sobel_op(outputs[:,0,:,:].unsqueeze(1)),sobel_op(outputs[:,1,:,:].unsqueeze(1))),dim=1)
        targets_edge = torch.cat((sobel_op(target[:,0,:,:].unsqueeze(1)),sobel_op(target[:,1,:,:].unsqueeze(1))),dim=1)
        # loss_edge = criterion(outputs_edge, targets_edge)
        loss_edge = fn_edge_loss(outputs_edge, targets_edge)
        loss_total += loss_edge * edge_loss_w
        loss_dict['edge'] = loss_edge.item()
         # print("sobel_kernel",sobel_op.weight_h)
    else:
        loss_dict['edge'] = 0
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, cap, keys,occm_mats) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # Update LR & WD for the first acc 正则化方法。学习率和权重衰减
        ...
        if loss_scaler is None:
            ...
            loss, output, loss_dict = train_class_batch_norearange(
                model, img_l.repeat(1,3,1,1), img_ab, cap, criterion, occm_mats, sobel_op=sobel_op) #这里的criterion设置成smooth_l1了 
        else:
...
def evaluate(data_loader, model, device, epoch=10000,patch_size=16,save_img_dir=None, istest = False):
    ...
    for step,(samples, cap, keys, occm_mat) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        # fake_ab_tensors = utils.lab2rgb(torch.cat((torch.zeros_like(img_l.type(torch.cuda.FloatTensor)), img_ab_fake), dim=1))
        # real_ab_tensors = utils.lab2rgb(torch.cat((torch.zeros_like(img_l.type(torch.cuda.FloatTensor)), img_ab), dim=1))
        # fake_ab = utils.tensor2im(fake_ab_tensors)
        # real_ab = utils.tensor2im(real_ab_tensors)
            ...
            if epoch%10 == 0:
                ...
                if not os.path.exists(output_path):
                    try:     
                        ...
                        pass
                if istest:
                    # output_path_fake = os.path.join(output_path,keys[i].split('.')[0]+ "_" + cap[i][0:200] + '.png')
                    output_path_fake = os.path.join(output_path,keys[i].split('.')[0]+ "_" + cap[i][0][0:200] + '.png')
        ...
        # 调试提前终止本轮test
        # break
    # psnrs_mean = np.mean(np.stack(psnrs_real))
    # print("psnr_mean",psnrs_mean)
    # gather the stats from all processes
