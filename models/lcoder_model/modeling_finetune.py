...
class Bert_encoder(nn.Module):
    def __init__(self,decoder_emb):
        ...
        local_model_filepath = 'models/local_bert-base-uncased'
        # model_name = 'bert-base-uncased'
        self.tokenizer = BertTokenizer.from_pretrained(local_model_filepath + '/tokenizer/')
        model_config = BertConfig.from_pretrained(local_model_filepath + '/config/')
        # 修改配置
        model_config.output_hidden_states = True
        # model_config.output_attentions = True
        self.bert_model = BertModel.from_pretrained(local_model_filepath + '/model/', 
                                                    config = model_config)
        ...
        # self.tokenizer.save_pretrained('bert-base-uncased/tokenizer')
        # self.bert_model.save_pretrained('local_bert-base-uncased/model')
        # model_config.save_pretrained('local_bert-base-uncased/config')
    ...
    def forward(self,txts,vis=None):
        ...
        # breakpoint()
        token_tensor = torch.LongTensor(token_ids).to(self.bert_model.device)
...
class Bert_encoder_onlyLanguage(nn.Module):
    def __init__(self,decoder_dim):
        ...
        #     hidden_size = 768,
        #     activation = nn.ReLU())
        # self.arc_biaffine = Biaffine(768, 768, 1, bias=(True, False))
