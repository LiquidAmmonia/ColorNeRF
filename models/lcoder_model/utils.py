...
def cosine_scheduler(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0,
    ...
    if warmup_epochs > 0:
        warmup_schedule = np.linspace(start_warmup_value, base_value, warmup_iters) #  线性空间中以均匀步长生成数字序列(start, end, num)
...
def rgb2lab(rgb):
    lab = xyz2lab(rgb2xyz(rgb)) # rgb2xyz函数有一步归一化: 0-255 -> 0-1,输入进来的rgb直接用ToTensor归一化的结果，
    l_rs = (lab[:,[0],:,:]-50.)/100. *2.# 取值范围[-1,1] default: l_cent = 50; l_norm = 100 
