...
class VisionTransformer(nn.Module):
    ...
    def forward(self, im, return_features=False, return_pos_emb=False):
        B, _, H, W = im.shape
        PS = self.patch_size
        ...
        if self.partial_finetune:
            with torch.no_grad():
                for blk_number in range(6):
                    x = self.blocks[blk_number](x)
            for blk_number in range(6, len(self.blocks)):
                x = self.blocks[blk_number](x)
        else:
